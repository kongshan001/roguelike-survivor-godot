...
func test_evolution_recipes_count():
	assert_eq(_registry.EVOLUTION_RECIPES.size(), 8, "Should have 8 evolution recipes")
...
func test_all_8_evolved_weapons_registered():
	_register_all_weapons()
	var expected := ["thunderholywater", "fireknife", "holydomain", "blizzard", "frostknife", "flamebible", "thunderang", "blazerang"]
	for id in expected:
		assert_ne(UpgradePool._weapons.get(id), null, "%s should be registered" % id)
