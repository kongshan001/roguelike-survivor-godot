...
基于R23的设计输出和v1.0.3路线图，建议R24的优先级排序：

| 优先级 | 任务 | 负责 Agent | 预估行数 | 阻塞项 |
|---|---|---|---|---|
| P1 | 武器精通暂停面板实现（hud_mastery_panel.gd ~80行） | Programmer | ~80实现 + ~30测试 | 无，后端+Toast已完成 |
| P1 | 教程步骤6-8实现（tutorial_manager.gd扩展） | Programmer | ~40实现 + ~30测试 | 无，TutorialManager已存在 |
| P2 | 进化武器Phase A+B注册（upgrade_pool + weapon_registry） | Programmer | ~43实现 + ~12测试 | 无 |
...
**建议R24范围**: 专注P1（精通暂停面板 + 教程扩展），视情况推进P2（进化注册）。P3推迟到v1.1.0。
